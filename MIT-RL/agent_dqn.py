...
def epsilon_greedy(state_vector, epsilon):
    ...
    if coin < epsilon:
        ...
        object_index = q_values_object_next.argmax().item()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    Returns:
        None
    """
    # print(next_state_vector.dtype)
    # print(model.state_encoder.weight.dtype)
    # print(model.state_encoder.bias.dtype)
    with torch.no_grad():
        q_values_action_next, q_values_object_next = model(torch.from_numpy(next_state_vector).to(torch.float32))
    ...
    q_values_cur_action, q_values_cur_object = model(current_state_vector)
    val = 1 / 2 * q_values_cur_action[action_index] + 1 / 2 * q_values_cur_object[object_index]
    loss = 0.5 * (target - val) ** 2 
