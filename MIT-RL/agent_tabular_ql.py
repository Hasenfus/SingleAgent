...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    """
    opt =  (np.random.randint(0,NUM_ACTIONS),np.random.randint(0,NUM_OBJECTS))
    # print(q_func.shape)
    # print(np.max(q_func[state_1,state_2,:,:]))
    explore =  np.where(q_func[state_1,state_2,:,:] == np.max(q_func[state_1,state_2,:,:]))
    explore = (explore[0][0],explore[1][0])
...
def run_episode(for_training):
    ...
    current_quest_ind = dict_quest_desc[current_quest_desc]
    ...
    while not terminal:
        ...
        current_room_ind = dict_room_desc[current_room_desc]
        current_quest_ind = dict_quest_desc[current_quest_desc]
        action, object = epsilon_greedy(current_room_ind,current_quest_ind,q_func,epsilon)
        ...
        if for_training:
            ...
            tabular_q_learning(q_func, current_room_ind,current_quest_ind,action, object, reward, next_room_ind, next_quest_ind, terminal)
            pass
...
if __name__ == '__main__':
    ...
    plt.show()
    print(np.max(np.mean(epoch_rewards_test,axis=0)))