...
TRAINING_EP = lambda x: np.max([0.01, 0.3 - 0.01*x])  # epsilon-greedy parameter for training
TESTING_EP = lambda x: 0.01  # epsilon-greedy parameter for testing
NUM_RUNS = 5
NUM_EPOCHS = 200
...
def epsilon_greedy(state_vector, epsilon, training=True):
    ...
    if coin < epsilon:
        action_index = np.random.randint(0,NUM_AC8TIONS)
    else:
        if training:
            with torch.no_grad():
...
class DQN(nn.Module):
    ...
    def __init__(self, state_dim, action_dim, hidden_size=4, hidden = False):
...
def run_episode(for_training, episode=0):
    """ Runs one episode
...
def run_epoch():
    ...
    for k in range(NUM_EPIS_TRAIN):
        run_episode(for_training=True, episode = k)
...
def run(breadth=False, hidden_size=4):
    """Returns array of test reward per epoch for one run"""
    global target
    global policy
    global optimizer
    target = DQN(NUM_STATES, NUM_ACTIONS, hidden=breadth, hidden_size=hidden_size)
    policy  = DQN(NUM_STATES, NUM_ACTIONS, hidden=breadth, hidden_size=hidden_size)
...
if __name__ == '__main__':
    ...
    epoch_rewards_all = []  # shape NUM_RUNS * NUM_EPOCHS
    models = []
    # model_params = [(False, 4), (True, 4), (True, 8), (True, 16)]
    ...
    for x in model_params:
        print("Running model: ", x)
        model_results = []
        epoch_rewards_model = []
        for _ in range(NUM_RUNS):
            rewards, model_output = run()
            epoch_rewards_model.append(rewards)
            model_results.append(model_output)
        models.append((x[0], x[1], model_results))
        epoch_rewards_all.append(epoch_rewards_model)
    ...
    if save:
        ...
        os.mkdir("runs/"+timestamp)
        #  Loop through models, choose best model for each run, save it
        for i in range(len(models)):
            j = np.argmax(epoch_rewards_all[i]) // NUM_EPOCHS
            torch.save(models[i][2][j].state_dict(), "results/" + timestamp + "/dqn_model_"+str(i)+".pt")
        epoch_rewards_all = np.array(epoch_rewards_all)
    # Loop through each model and plot the average reward per epoch
    for i in range(len(models)):
        x = np.arange(NUM_EPOCHS)
        fig, axis = plt.subplots()
        axis.plot(x, np.mean(epoch_rewards_all[i],
                            axis=0))  # plot reward per epoch averaged per run
        axis.set_xlabel('Epochs')
        axis.set_ylabel('reward')
        # axis.set_title(('Linear: nRuns=%d, Epsilon=%.2f, Epi=%d, alpha=%.4f' %
        #                 (NUM_RUNS, TRAINING_EP, NUM_EPIS_TRAIN, ALPHA)))
        axis.set_title(('DQN: nRuns=%d, Epsilon=%.2f, Epi=%d, alpha=%.4f, breadth=%s, hidden_size=%d' %
                        (NUM_RUNS, TRAINING_EP, NUM_EPIS_TRAIN, ALPHA, models[i][0], models[i][1])))
        if save:
            plt.savefig("runs/" + timestamp +"/dqn_"+str(i)+".png")
        else:
            plt.show()
